...
class CryptoRadioDJ {
  ...
  async getChannelMessages(channel, limit = 10) {
    try {
      ...
      console.error(`❌ Ошибка получения сообщений из ${channel.title}:`, error.message);
      return [];
    }
  }
  ...
  async collectAllMessages() {
    ...
    for (const channel of this.targetChannels) {
      try {
        const channelEntity = await this.client.getEntity({
          _: 'inputPeerChannel',
          channelId: channel.id,
          accessHash: channel.accessHash
        });

        const messages = await this.getChannelMessages(channelEntity, 10);
        allMessages.push(...messages);
        console.log(`✅ Получено ${messages.length} сообщений из ${channel.actualTitle}`);
      } catch (error) {
        console.error(`❌ Ошибка с каналом ${channel.actualTitle}:`, error.message);
      }
    }
